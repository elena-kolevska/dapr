...
type ActorsManager interface {
	RegisterActor(ctx context.Context) error
	UnRegisterActor(ctx context.Context) error

	StartScheduling()
	StopScheduling()
}

var _ ActorsManager = &ActorBackend{}
...
func (abe *ActorBackend) StartScheduling() {
	abe.schedulingGate.Store(true)
}
func (abe *ActorBackend) StopScheduling() {
	abe.schedulingGate.Store(false)
}
...
func (abe *ActorBackend) UnRegisterActor(ctx context.Context) error {
	if abe.actorRuntime != nil {
		errs := []error{}
		for actorType := range abe.GetInternalActorsMap() {
			err := abe.actorRuntime.UnregisterInternalActor(ctx, actorType)
			if err != nil {
				errs = append(errs, fmt.Errorf("failed to unregister workflow actor %s: %w", actorType, err))
			}
		}
		return errors.Join(errs...)
	}
...
func (abe *ActorBackend) GetActivityWorkItem(ctx context.Context) (*backend.ActivityWorkItem, error) {
	if !abe.schedulingGate.Load() {
		return nil, errors.New("scheduling is stopped")
	}
